...
** advanced-build
This might be tricky, but the idea is:
 - extract deps.edn
 - parse the org file
 - replace any rendering functions according to user specification (for example, svg! function only makes sense in CLJ context as it spits file to disk... doesn't work in browser, but could have a different impl that makes sense for rendering OR is passed as identity...)
 - create the cljs reagent project file which will be compiled into the doc.
